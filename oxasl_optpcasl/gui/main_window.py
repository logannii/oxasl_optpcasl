...
from .sensitivity_plot import ATTSensitivityPlot, CBFSensitivityPlot, KineticCurve
...
class OptPCASLGui(wx.Frame):
    ...
    def __init__(self):
        ...
        icon = wx.Icon()
        icon.CopyFromBitmap(
            wx.Bitmap(os.path.join(local_dir, "icon.png"), wx.BITMAP_TYPE_ANY)
        )
        self.SetIcon(icon)
    ...
    def optimize(self, niters=1):
        ...
        self._runner.run(
            ...
            reps=niters,
            gridpts=self.opt.gridpts,
        )

    def generate_report(self, report_path):
        """ """
        report = Report(
            "OXASL PCASL optimizer - Protocol report", include_timings=False
        )
    ...
    try:
        ...
        if sys.platform == "darwin" and "Framework" in str(exc):
            ...
            if not os.path.exists(conda_python_app):
                sys.stderr.write(
                    "You seem to have a Conda installation on Mac. We need\n"
                )
                sys.stderr.write(
                    "an extra package to run GUIs on Mac Conda. Please try:\n\n"
                )
                ...
                launcher = sys.argv[0]
                sys.stderr.write(
                    "WARNING: We have found a known problem with running GUI\n"
                )
                sys.stderr.write(
                    "applications on Mac. I will try to fix it now by modifying\n"
                )
                ...
                try:
                    ...
                    if patched:
                        ...
                        sys.stderr.write(
                            "FAILED - file listed above does not look like a normal GUI launcher\n"
                        )
                        sys.stderr.write(
                            "Re-throwing original error - you may need to manually run this\n"
                        )
                        sys.stderr.write(
                            "progam using pythonw or %s instead of just python\n"
                            % conda_python_app
                        )
                        raise
                except PermissionError:
                    sys.stderr.write(
                        "FAILED to modify the launcher file because of insufficient permissions\n"
                    )
                    sys.stderr.write("You can try either of the following\n")
                    sys.stderr.write(
                        " - Running this program ONCE using sudo (i.e. sudo %s)\n"
                        % launcher
                    )
                    sys.stderr.write(
                        " - OR editing the file above as an administrator and replacing\n"
                    )
