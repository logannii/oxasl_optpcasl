...
class ScanParams(object):
    ...
    def __init__(
        ...
        if self.plds is not None and len(self.plds) != self.npld:
            raise ValueError(
                "Supplied initial PLDs %s is inconsistent with stated number of PLDs: %i"
                % (plds, npld)
            )
...
class ATTDist(object):
    ...
    def __init__(self, start=0.5, end=2.0, step=0.025, taper=0):
        ...
        taper_points = (
            int(np.ceil(taper / step)) - 1
        )  # FIXME I don't agree with this but it is compatible with Joe's code
        self.start, self.end, self.step, self.taper = start, end, step, taper
        self.atts = np.linspace(start, end, total_points)
        self.exclude_taper = np.linspace(
            start + taper, end - taper, total_points - 2 * taper_points
        )
    ...
    def __str__(self):
        return (
            "ATT distribution: %i values between %.3fs and %.3fs (weight taper=%.3fs)"
            % (self.length, self.start, self.end, self.taper)
        )
...
class Limits(object):
    ...
    def __str__(self):
        return "%ss between %.3fs and %.3fs in steps of %.5fs" % (
            self.name,
            self.lb,
            self.ub,
            self.step,
